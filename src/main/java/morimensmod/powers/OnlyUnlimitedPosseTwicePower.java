...
public class OnlyUnlimitedPosseTwicePower extends AbstractEasyPower implements OnAfterPosse {

    private static final Logger logger = LogManager.getLogger(OnlyUnlimitedPosseTwicePower.class);
    ...
    public OnlyUnlimitedPosseTwicePower(AbstractCreature owner, int amount) {
    ...
    public void onAfterPosse(AbstractPosse posse, int exhaustKeyflare) {
        if (posse.getType() != PosseType.UNLIMITED)
            return;

        logger.debug("posse:" + posse.cardID);
